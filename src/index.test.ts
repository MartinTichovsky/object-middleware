...
import {
    INSTANCE_INIT_INDEX,
    INSTANCE_ORIGIN_INDEX,
    ObjectMiddlewareType,
    PROTOTYPE_INIT_INDEX,
    subscribe,
    subscribeTypeSafe,
    unsubscribe,
    unsubscribeAll
...
describe("Subscribe", () => {
    test("All Methods Must Have Middleware", () => {
        ...
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(methodsCount);

        subscribe(myObject, middleware, ObjectMiddlewareType.BEFORE);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(methodsCount * 2);

        subscribe(myObject, middleware, ObjectMiddlewareType.CONDITION_AFTER);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(methodsCount * 3);

        subscribe(myObject, middleware, ObjectMiddlewareType.CONDITION_BEFORE);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(methodsCount * 4);

        subscribe(myObject, middleware, ObjectMiddlewareType.OVERRIDE);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(methodsCount * 5);
    });
    ...
    test("Only Selection of Methods Must Have Middleware", () => {
        ...
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(1);

        subscribe(myObject, middleware, ObjectMiddlewareType.BEFORE, methodList);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(2);

        subscribe(myObject, middleware, ObjectMiddlewareType.CONDITION_AFTER, methodList);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(3);

        subscribe(myObject, middleware, ObjectMiddlewareType.CONDITION_BEFORE, methodList);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(4);

        subscribe(myObject, middleware, ObjectMiddlewareType.OVERRIDE, "method");
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(5);
    });
});
...
describe("Unsubscribe", () => {
    test("Main Functionality", () => {
        ...
        expect(
            (myObject[INSTANCE_INIT_INDEX] as ObjectMiddlewareInitType<unknown>[]).filter(
    ...
    test("Origin Methods Mustn't Remain", () => {
        ...
        subscribe(myObject, middleware, ObjectMiddlewareType.AFTER);
        unsubscribe(myObject, middleware, ObjectMiddlewareType.AFTER);
        expect(myObject[INSTANCE_ORIGIN_INDEX].length).toBe(0);
    });
...
describe("Unsubscribe All", () => {
    test("Main Functionality", () => {
        ...
        expect(myObject1[INSTANCE_INIT_INDEX].length).toBe(0);
        expect(myObject2[INSTANCE_INIT_INDEX]).toBe(undefined);

        subscribe(myObject1, middleware, ObjectMiddlewareType.AFTER);
        subscribe(myObject2, middleware, ObjectMiddlewareType.AFTER);
        unsubscribeAll(myObject1);
        expect(myObject1[INSTANCE_INIT_INDEX].length).toBe(0);
        expect(myObject2[INSTANCE_INIT_INDEX].length).toBe(methodsCount);

        unsubscribeAll(myObject2);
        expect(myObject2[INSTANCE_INIT_INDEX].length).toBe(0);
    ...
    test("Unsubscribe Methods In List", () => {
        ...
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(methodsCount - 1);
        unsubscribeAll(myObject);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe(0);

        subscribe(myObject, middleware, ObjectMiddlewareType.AFTER);
        subscribe(myObject, middleware, ObjectMiddlewareType.BEFORE);
        unsubscribeAll(myObject, [myObject.method1.name]);
        expect(myObject[INSTANCE_INIT_INDEX].length).toBe((methodsCount - 1) * 2);
    });
});
...
describe("Returns", () => {
    ...
    test("Override Middleware", () => {
        ...
        expect(myObject.method3(params[0], params[1], params[2])).toEqual(params.reverse());
    });
...
describe("Parameters Must be Passed", () => {
    const myObject = new MyClass();
    const params: [string, number, boolean, symbol] = ["A", 1, false, Symbol("__unique__")];
    const middleware: ObjectMiddlewareFunction<MyClass> = function (
        _,
        param1: string,
        param2: number,
...
describe("Prototype", () => {
    ...
    test("Unsubscribe", () => {
        ...
        expect(testObject1[PROTOTYPE_INIT_INDEX].length).toBe(methodsCount);
        expect(testObject2[PROTOTYPE_INIT_INDEX].length).toBe(methodsCount);

        unsubscribeAll(testObject1, [], true);

        expect(testObject1[PROTOTYPE_INIT_INDEX].length).toBe(0);
        expect(testObject2[PROTOTYPE_INIT_INDEX].length).toBe(0);
    });
    ...
    test("Shouldn't affect instances", () => {
        ...
        subscribe(testObject1, middleware1, ObjectMiddlewareType.OVERRIDE, [], true);

        subscribe(testObject2, middleware2, ObjectMiddlewareType.OVERRIDE);

        expect(testObject1[PROTOTYPE_INIT_INDEX].length).toBe(methodsCount);
        expect(testObject1[INSTANCE_INIT_INDEX]).toBe(undefined);
        expect(testObject2[PROTOTYPE_INIT_INDEX].length).toBe(methodsCount);
        expect(testObject2[INSTANCE_INIT_INDEX].length).toBe(methodsCount);
...
test("Errors", () => {
    ...
    unsubscribe("", middleware, ObjectMiddlewareType.OVERRIDE);
    unsubscribe(testObject, "" as any, ObjectMiddlewareType.OVERRIDE);

    unsubscribeAll("");

    expect(() => subscribe([], middleware)).toThrow();
    expect(() => subscribe(null as any, middleware)).toThrow();
    expect(() => subscribe("" as any, middleware)).toThrow();
    expect(() => subscribe(testObject, "" as any)).toThrow();
});
...
describe("TypeScript safe using", () => {
    test("Class", () => {
        ...
        subscribeTypeSafe(myObject, middleware, "myMethod", ObjectMiddlewareType.OVERRIDE);
